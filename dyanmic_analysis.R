...
for (i in c(1:2459)) {
  cry_trad <- as.numeric(sp$list_of_tables[[i]][[1]][[1]][c(6:14),c(1:5)])
  trad_cry <- as.numeric(sp$list_of_tables[[i]][[1]][[1]][c(1:5),c(6:14)])
  p_cry_trad_s <- append(p_cry_trad_s, sum(cry_trad) / 45)
  p_trad_cry_s <- append(p_trad_cry_s, sum(trad_cry) / 45)
}
p_cry_trad_m <- vector(mode="numeric", length=0)
p_trad_cry_m <- vector(mode="numeric", length=0)
for (i in c(1:2459)) {
  cry_trad <- as.numeric(sp$list_of_tables[[i]][[1]][[2]][c(6:14),c(1:5)])
  trad_cry <- as.numeric(sp$list_of_tables[[i]][[1]][[2]][c(1:5),c(6:14)])
  p_cry_trad_m <- append(p_cry_trad_m, sum(cry_trad) / 45)
  p_trad_cry_m <- append(p_trad_cry_m, sum(trad_cry) / 45)
}
p_cry_trad_l <- vector(mode="numeric", length=0)
p_trad_cry_l <- vector(mode="numeric", length=0)
for (i in c(1:2459)) {
  cry_trad <- as.numeric(sp$list_of_tables[[i]][[1]][[3]][c(6:14),c(1:5)])
  trad_cry <- as.numeric(sp$list_of_tables[[i]][[1]][[3]][c(1:5),c(6:14)])
  p_cry_trad_l <- append(p_cry_trad_l, sum(cry_trad) / 45)
  p_trad_cry_l <- append(p_trad_cry_l, sum(trad_cry) / 45)
}
dates <- seq(as.Date("2015-1-1"), as.Date("2022-1-1"), by = "days")
colors <- c("Curren->Crypto" = "#213FE7", "Crypto->Curren" = "#D82121")

a1 <- ggplot() +
  geom_line(aes(x=dates[c(1:2459)], y=p_cry_trad_s, color='Crypto->Curren'), size=1) +
  geom_line(aes(x=dates[c(1:2459)], y=p_trad_cry_s, color='Curren->Crypto'), size=1) +
  labs(y="Connectedness", x="Time", color='') +
  scale_color_manual(values = colors) + theme_cowplot() +
  theme(legend.position = c(0.4, 0.9),
        text=element_text(family="Times New Roman"))
a2 <- ggplot() +
  geom_line(aes(x=dates[c(1:2459)], y=p_cry_trad_m, color='Crypto->Curren'), size=1) +
  geom_line(aes(x=dates[c(1:2459)], y=p_trad_cry_m, color='Curren->Crypto'), size=1) +
  labs(y="Connectedness", x="Time", color='') +
  scale_color_manual(values = colors) + theme_cowplot() +
  theme(legend.position = c(0.4, 0.9),
        text=element_text(family="Times New Roman"))
a3 <- ggplot() +
  geom_line(aes(x=dates[c(1:2459)], y=p_cry_trad_l, color='Crypto->Curren'), size=1) +
  geom_line(aes(x=dates[c(1:2459)], y=p_trad_cry_l, color='Curren->Crypto'), size=1) +
  labs(y="Connectedness", x="Time", color='') +
  scale_color_manual(values = colors) + theme_cowplot() +
  theme(legend.position = c(0.4, 0.9),
        text=element_text(family="Times New Roman"))
